...
function scanBoxes(buf: Uint8Array, start: number, end: number): Box[] {
  const boxes: Box[] = [];
  let pos = start;
  while (pos + 8 <= end) {
...
function findPath(buf: Uint8Array, boxes: Box[], path: string[]): Box | undefined {
  let current = boxes;
...
function parseTrack(buf: Uint8Array, trakBox: Box, mdatDataOffset: number): TrackInfo | null {
