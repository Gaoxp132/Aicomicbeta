/**
 * SeriesListWidgets.tsx — Sub-components for SeriesListView
 * Extracted from SeriesListView.tsx: EmptyState, SeriesDownloadMenu, SeriesCard,
 * SeriesDetailModal, SeriesSearchBar, and helper functions.
 */
...
export function EmptyState({ type, onCreateNew, userPhone }: { type: 'no-login' | 'no-series' | 'no-results'; onCreateNew?: () => void; userPhone?: string }) {
  if (type === 'no-login') return (<div className="bg-white/5 backdrop-blur-xl rounded-3xl p-12 border border-white/10 text-center"><Film className="w-16 h-16 text-gray-500 mx-auto mb-4" /><h3 className="text-xl font-semibold text-white mb-2">请先登录</h3><p className="text-gray-400">登录后即可创作和管理您的影视作品</p></div>);
  if (type === 'no-series') return (<div className="bg-white/5 backdrop-blur-xl rounded-3xl p-12 border border-white/10 text-center"><div className="w-20 h-20 bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-full flex items-center justify-center mx-auto mb-6"><Film className="w-10 h-10 text-purple-400" /></div><h3 className="text-xl font-semibold text-white mb-2">还没有影视作品</h3><p className="text-gray-400 mb-6">开始创作您的第一部AI影视作品吧！</p><Button onClick={onCreateNew} className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600">创建新作品</Button></div>);
  if (type === 'no-results') return (<div className="bg-white/5 backdrop-blur-xl rounded-3xl p-12 border border-white/10 text-center"><Search className="w-16 h-16 text-gray-500 mx-auto mb-4" /><h3 className="text-xl font-semibold text-white mb-2">未找到匹配的作品</h3><p className="text-gray-400">尝试更改搜索条件或筛选器</p></div>);
  return null;
}

// ── SeriesDownloadMenu ───────────────────────────────────────────────

export function SeriesDownloadMenu({ onDownload, onDownloadVideos, onClose }: { series: Series; onDownload: (f: 'json' | 'txt' | 'html') => void; onDownloadVideos: () => void; onClose: () => void }) {
  return (
...
const GENRE_ICONS: Record<string, string> = { romance: '\u{1F495}', suspense: '\u{1F50D}', comedy: '\u{1F604}', action: '\u26A1', fantasy: '\u2728', horror: '\u{1F47B}', scifi: '\u{1F680}', drama: '\u{1F3AD}' };
...
export function computeEffectiveStatus(series: Series): string {
  if (series.status === 'failed' || series.status === 'generating') {
    const eps = series.episodes || [];
    if (eps.length > 0 && (series.totalEpisodes <= 0 || eps.length >= series.totalEpisodes)) {
      ...
      if (progress && typeof progress.currentStep === 'number' && progress.currentStep >= 5) {
        const allEpsHaveStoryboards = eps.every(ep => ep.storyboards && ep.storyboards.length > 0);
        if (allEpsHaveStoryboards) return 'completed';
      }
      const allDone = eps.every(ep => ep.storyboards && ep.storyboards.length > 0 && ep.status !== 'generating' && ep.storyboards.every(sb => sb.imageUrl || sb.thumbnailUrl));
      if (allDone) return 'completed';
    }
    return series.status;
  }
  if (series.status === 'completed' || series.status === 'in-progress') return series.status;
  const episodes = series.episodes || [];
  if (episodes.length === 0) return series.status;
  const hasContent = episodes.some(ep => ep.storyboards && ep.storyboards.length > 0);
  if (!hasContent) return series.status;
  if (episodes.length > 0 && hasContent) return 'completed';
  return series.status;
}

// ── SeriesCard ───────────────────────────────────────────────────────

export function SeriesCard({ series, onEdit, onDelete, onShowDetail, onDownload, onDownloadVideos, onRetry, showDownloadMenu, onToggleDownloadMenu }: { series: Series; onEdit: (s: Series) => void; onDelete: (id: string, e: React.MouseEvent) => void; onShowDetail: (s: Series) => void; onDownload: (s: Series, f: 'json' | 'txt' | 'html') => void; onDownloadVideos: (s: Series) => void; onRetry: (id: string, outline: string) => void; showDownloadMenu: boolean; onToggleDownloadMenu: (e: React.MouseEvent) => void; isDownloading: boolean }) {
  ...
  const genreGradient = GENRE_GRADIENTS[series.genre] || 'from-purple-600/40 via-pink-500/30 to-rose-500/20'; const genreIcon = GENRE_ICONS[series.genre] || '\u{1F3AC}';
      ...
      <div className="p-4">
        ...
        {isGenerating && (
          <div className={`mb-3 p-3 border rounded-xl ${isStuck ? 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 border-amber-500/30' : 'bg-gradient-to-r from-purple-500/20 to-pink-500/20 border-purple-500/30'}`}>
            ...
            : series.queueStatus === 'queued' ? <div className="mt-2 text-xs text-gray-400">您有其他作品正在生成，请稍候...</div>
...
export function SeriesDetailModal({ series, onClose, onEdit }: { series: Series; onClose: () => void; onEdit: (s: Series) => void }) {
  const [playingEpisode, setPlayingEpisode] = useState<{ id: string; title: string } | null>(null);
  return (
    <> {/* modal */}
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="bg-gradient-to-br from-gray-900 to-purple-900 rounded-3xl border border-white/10 max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {series.characters && series.characters.length > 0 && <div><h3 className="text-lg font-semibold text-white mb-3">角色列表</h3><div className="space-y-3">{series.characters.map((char: { name: string; description?: string; growthArc?: string }, idx: number) => <div key={idx} className="bg-white/5 p-4 rounded-xl"><div className="font-semibold text-white mb-2">{char.name}</div><div className="text-gray-400 text-sm">{char.description}</div>{char.growthArc && <div className="mt-2 text-purple-300 text-sm">成长轨迹：{char.growthArc}</div>}</div>)}</div></div>}
            {series.episodes && series.episodes.length > 0 && <div><h3 className="text-lg font-semibold text-white mb-3">剧集列表</h3><div className="space-y-3">{series.episodes.map((ep: { id: string; episodeNumber: number; title: string; synopsis?: string; growthTheme?: string; storyboards?: { length: number }[] }) => <div key={ep.id} className="bg-white/5 p-4 rounded-xl"><div className="font-semibold text-white mb-2">第{ep.episodeNumber}集：{ep.title}</div><div className="text-gray-400 text-sm mb-2">{ep.synopsis}</div>{ep.growthTheme && <div className="text-purple-300 text-sm">成长主题：{ep.growthTheme}</div>}{ep.storyboards && <div className="mt-2 text-gray-500 text-xs">{ep.storyboards.length} 个分镜场景</div>}<Button onClick={() => setPlayingEpisode({ id: ep.id, title: ep.title })} className="mt-3 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white"><Play className="w-4 h-4 mr-2" />播放剧集</Button></div>)}</div></div>}
          </div>
...
export function SeriesSearchBar({ searchTerm, onSearchChange, filterStatus, onFilterChange, resultCount }: { searchTerm: string; onSearchChange: (v: string) => void; filterStatus: 'all' | 'draft' | 'in-progress' | 'completed'; onFilterChange: (s: 'all' | 'draft' | 'in-progress' | 'completed') => void; resultCount: number }) {
