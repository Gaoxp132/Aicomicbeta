...
// [V-D] ViewerFinale
export function ViewerFinale({ series, episodes, similarSeries, onReplay, onSelectEpisode, onClose, onNavigateToSeries }: { series: { id: string; title: string }; episodes: { id: string; episodeNumber: number; title: string }[]; similarSeries: { id: string; title: string; coverImage?: string; totalEpisodes?: number; likes?: number }[]; isLoadingSimilar?: boolean; onReplay: () => void; onSelectEpisode: () => void; onClose: () => void; onNavigateToSeries?: (id: string) => void }) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-md overflow-y-auto">
      <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.85, opacity: 0 }} transition={{ type: 'spring', damping: 22, stiffness: 260 }} className="max-w-md mx-4 my-8 p-8 bg-gradient-to-br from-purple-900/60 via-slate-900/60 to-pink-900/60 backdrop-blur-2xl rounded-3xl border border-white/15 shadow-2xl text-center">
        ...
        {similarSeries.length > 0 && <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-6 pt-5 border-t border-white/10"><p className="text-gray-400 text-xs mb-3 flex items-center justify-center gap-1"><ChevronRight className="w-3 h-3" />猜你也会喜欢</p><div className="grid grid-cols-2 gap-2">{similarSeries.map((sim: { id: string; title: string; coverImage?: string; totalEpisodes?: number; likes?: number }) => <button key={sim.id} onClick={() => { onClose(); onNavigateToSeries ? onNavigateToSeries(sim.id) : (window.location.hash = `#series-${sim.id}`); }} className="p-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/15 transition-all text-left group">{sim.coverImage && <div className="w-full aspect-[16/9] rounded-lg overflow-hidden mb-2 bg-white/5"><img src={sim.coverImage} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /></div>}<h4 className="text-white text-xs font-medium truncate">{sim.title}</h4><div className="flex items-center gap-2 mt-1"><span className="text-gray-500 text-[10px]">{sim.totalEpisodes}集</span>{(sim.likes ?? 0) > 0 && <span className="flex items-center gap-0.5 text-gray-500 text-[10px]"><Heart className="w-2.5 h-2.5" />{sim.likes}</span>}</div></button>)}</div></motion.div>}
      </motion.div>
    </motion.div>
  );
}
